...
#
# Tags look like `v<Matomo version>-<release>`:
#
# - if defaults/main.yml points at a Matomo version that has never been
#   released, the release counter restarts at 0 (`v5.13.0-0`)
# - otherwise the counter is incremented (`v5.13.0-1`), but only if something
...
# Anchored on `matomo_version:` so that neither `matomo_container_image_tag:`
# (which names it in a Jinja expression) nor a commented-out line can be
# mistaken for it.
version="$(sed -nE 's|^matomo_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Matomo version from $defaults_path"
	exit 1
fi

# The value in defaults/main.yml carries no leading `v` (e.g. `5.13.0`) while
# the tags do (`v5.13.0-0`). Stripping first keeps this working either way.
